...
fwd1(uint64_t *a, const mul_op_m512_t w[5], const uint64_t q_64)
{
  ...
  __m512i X = GATHER(idx, &a[0 + 0], 8);
  __m512i Y = GATHER(idx, &a[0 + 1], 8);
  __m512i Z = GATHER(idx, &a[0 + 2], 8);
  __m512i T = GATHER(idx, &a[0 + 3], 8);

  fwd_radix4_butterfly_m512(&X, &Y, &Z, &T, w, q_64);

  SCATTER(&a[0 + 0], idx, X, 8);
  SCATTER(&a[0 + 1], idx, Y, 8);
  SCATTER(&a[0 + 2], idx, Z, 8);
  SCATTER(&a[0 + 3], idx, T, 8);
}
...
fwd4(uint64_t *a, const mul_op_m512_t w[5], const uint64_t q_64)
{
  ...
  __m512i X = SHUF(X1, Z1, 0x44);
  __m512i Y = SHUF(X1, Z1, 0xee);
  __m512i Z = SHUF(Y1, T1, 0x44);
  __m512i T = SHUF(Y1, T1, 0xee);
